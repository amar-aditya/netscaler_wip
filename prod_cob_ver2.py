...
def prodcob():
        with open ("New_domain.txt",'r') as d_name :
                ...
                for domain in data:
			domain=domain.strip('\n')
#                        prod_vs_value =  (os.popen("cat ns.conf | grep -w "+ domain + " | cut -d ' ' -f4 | head -1" ).read()).strip('\n')
			prod_vs_value =  (os.popen("cat ns.conf | grep -E \"(^|\s)"+domain+"($|\s)\" | cut -d ' ' -f4 | head -1" ).read()).strip('\n')     
			bckp_vs_value = (os.popen("cat ns.conf | grep -E  \"(^|\s)"+prod_vs_value+"($|\s)\" | grep -i 'backupVserver' | cut -d ' ' -f6  | head -1").read()).strip('\n')
                        #print (bckp_vs_value)
